...
// TestEveryRouteIsInTheContract holds the routes the runtime gateway serves
...
func TestEveryRouteIsInTheContract(t *testing.T) {
	...
	// Every route is mounted behind the internal token check.
	if !slices.Equal(mounts, []string{"/api/", "/gateway/"}) || len(top.patterns) != 2 {
