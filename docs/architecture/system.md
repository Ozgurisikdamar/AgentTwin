...
| `evaluation` | evaluation-service | dataset, dataset_version, eval_run, eval_run_transition, eval_case_result, judgment (verdict cache), human_review, judge_calibration, outbox, processed_event; regression mining (ADR-0032): regression_group, regression_fingerprint, regression_occurrence (features + pgvector embedding, no content), regression_flag, regression_event |
