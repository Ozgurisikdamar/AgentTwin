"""The ``simulation-service`` command answers ``healthcheck`` without importing the
...
from agenttwin_simulation import cli
from agenttwin_simulation.main import SPEC

HEAVY = ("fastapi", "psycopg", "uvicorn", "agenttwin_simulation.main")
# Runs the command in a fresh interpreter and reports what it imported.
PROGRAM = f"""
import sys
from agenttwin_simulation.cli import main
sys.argv = ["simulation-service", *sys.argv[1:]]
try:
    main()
finally:
...
def test_every_other_command_runs_the_service() -> None:
    ...
    assert code == 2
    assert "usage: simulation-service" in err
